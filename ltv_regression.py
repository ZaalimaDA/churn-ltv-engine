"""
Week 3, Day 1-3: LTV (Lifetime Value) Regression Modeling
Project: Customer Churn & LTV Engine

What is LTV?
  Lifetime Value = the total revenue a business can expect from a customer
  over the entire duration of their relationship.

  For ACTIVE (non-churned) customers we FORECAST their expected future revenue.
  For ALL customers we also compute a HISTORICAL LTV baseline.

Approach:
  - Historical LTV  : total_charges (what they have already paid)
  - Projected LTV   : regression model predicting expected future revenue
                      based on tenure, services, contract type, charges

Regression Models:
  1. Linear Regression   — interpretable baseline
  2. Random Forest Regressor — handles non-linearity
  3. XGBoost Regressor   — best performance benchmark

Outputs:
  - ltv_outputs/ folder with 7 plots
  - customers_ltv table in PostgreSQL
  - ltv_model_metrics table in PostgreSQL
  - saved_models/ltv_xgboost.pkl (best model saved)
"""
...
# ════════════════════════════════════════════════════════════════════════════
# PART 7 — SAVE TO POSTGRESQL
# ════════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 65)
print("PART 7 — SAVE TO POSTGRESQL")
print("=" * 65)

# Save customers_ltv table
ltv_save_cols = [
    "customer_id", "tenure", "contract", "monthly_charges",
    "total_charges", "churn",
    "ltv_historical", "ltv_projected", "ltv_predicted",
    "ltv_segment",
    "charge_per_tenure", "service_count",
    "charge_to_value_ratio", "support_dependency_score",
    "tenure_contract_risk",
]
ltv_save_cols = [c for c in ltv_save_cols if c in df.columns]
df_ltv = df[ltv_save_cols].copy()
df_ltv["ltv_segment"] = df_ltv["ltv_segment"].astype(str)
df_ltv.to_sql("customers_ltv", engine, if_exists="replace", index=False)

with engine.connect() as conn:
    count = conn.execute(
        text("SELECT COUNT(*) FROM customers_ltv")).scalar()
print(f"\nSaved customers_ltv : {count} rows, {len(ltv_save_cols)} cols")
...
metrics_df.to_sql("ltv_model_metrics", engine,
                  if_exists="replace", index=False)
print(f"Saved ltv_model_metrics : {len(metrics_df)} rows")
print(metrics_df.to_string(index=False))

# ── Final Summary ─────────────────────────────────────────────────────────
lr_m   = reg_results["Linear Regression"]
rf_m   = reg_results["Random Forest"]
xgb_m  = reg_results["XGBoost"]

print(f"""
╔══════════════════════════════════════════════════════════════════╗
║        WEEK 3 DAY 1-3 — LTV REGRESSION COMPLETE                 ║
╠══════════════════════════════════════════════════════════════════╣
║  LTV Target     : ltv_projected (historical + contract estimate) ║
║  Features       : 5 engineered + numeric + encoded cats          ║
╠══════════════════════════════════════════════════════════════════╣
║  MODEL RESULTS:                                                  ║
║  Model                MAE ($)   RMSE ($)    R²     MAPE (%)      ║
║  Linear Regression   {lr_m['mae']:>8,.0f}  {lr_m['rmse']:>8,.0f}  {lr_m['r2']:.4f}   {lr_m['mape']:>6.2f}     ║
║  Random Forest       {rf_m['mae']:>8,.0f}  {rf_m['rmse']:>8,.0f}  {rf_m['r2']:.4f}   {rf_m['mape']:>6.2f}     ║
║  XGBoost             {xgb_m['mae']:>8,.0f}  {xgb_m['rmse']:>8,.0f}  {xgb_m['r2']:.4f}   {xgb_m['mape']:>6.2f}     ║
╠══════════════════════════════════════════════════════════════════╣
║  Best Model (R²) : {best_name:<44s}║
╠══════════════════════════════════════════════════════════════════╣
║  Plots  : ltv_outputs/  (7 plots)                                ║
║  Models : saved_models/ (ltv_linear_regression, rf, xgboost)    ║
║  Tables : customers_ltv, ltv_model_metrics                       ║
╚══════════════════════════════════════════════════════════════════╝

  Next: Week 3 Day 4-7 — FastAPI service development
""")
